...
*(The second half of that paragraph is now out of date: O3's parity half was accepted and
fixed — see the next section. The first half still stands.)*

## O3: the gate that could not see a dropped `NOT NULL`

**The gap.** `SchemaParityTest` parsed every column line with a pattern that captured
name, type and declared length only, and kept the length. The words `NOT NULL` were never
read. So `vet_id INT NOT NULL` in one dialect and `vet_id INT` in another were, to the
harness, the same column. A later change that dropped `NOT NULL` from exactly one of the
three schema files passed every gate in HARNESS.md — including the one whose whole job is
to notice that a schema change landed in one database and not the others. This slice put
three such columns in the ground: `vet_id`, `start_time` and `visit_date`.

**Why it mattered here rather than in general.** D2 above takes a decision away from a
later slice (S4), and the stated argument for doing so is that reversing it would cost a
visible, three-dialect migration. That argument rests entirely on the drift being
impossible to make quietly. Until this fix, it was not: one dialect could lose its
`NOT NULL` and nothing would say so. D2's cost-of-reversal reasoning now has something
behind it.

**How it was found.** Same route as O1: not by the code review, which returned PASS on
this change, but by the code-mode `/diaboli` pass afterwards, as objection **O3** in
[`docs/superpowers/objections/visit-carries-vet-and-time-code.md`](../docs/superpowers/objections/visit-carries-vet-and-time-code.md).
The maintainer accepted it and asked for the check. O4–O7 in that file remain with the
maintainer and are not acted on here.

**The fix.** `SchemaParityTest` now records each column as a `Column(length, notNull)`
rather than a bare length, and a third check, `nullabilityAgreesAcrossDialects`, compares
nullability for every column two dialects both declare. It runs over all three pairs —
h2/mysql, h2/postgres, mysql/postgres — rather than h2 against the others, because a
column can exist in two dialects and not the third and the disagreement is still real.

Two pieces of dialect syntax had to be read rather than assumed:

- `NOT NULL` sits in a different place in each dialect — postgres writes
  `vet_id INT NOT NULL REFERENCES vets (id)`, mysql writes
  `id INT(4) UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY` — so the whole column line is
  searched rather than a fixed position after the type.
- **`PRIMARY KEY` is counted as `NOT NULL`**, because it is one in every database here
  whether or not the dialect spells it out. h2 and postgres write
  `INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY` and leave it implicit; mysql
  writes it out. Reading only the literal words would report every `id` column in the
  schema as a disagreement — a false failure on day one, which is how a check gets
  ignored. The reason is written into the test beside the pattern, not only here.

**It was proved before it was believed.** `NOT NULL` was temporarily dropped from
`visit_date` in `db/postgres/schema.sql` alone and the test run. It failed with:

```
Nullability disagrees between dialects — a row one database would reject is stored happily by another, and code written against the stricter one breaks on the looser:
  visits.visit_date: h2 declares NOT NULL, postgres allows null
  visits.visit_date: mysql declares NOT NULL, postgres allows null

HARNESS.md: schema changes land in h2, mysql and postgres in the same change.
```

The schema file was then restored. No schema file is changed by this fix.

**What the check still does not cover, stated plainly.**

- It compares the three **files to each other**. It does not assert that any database
  rejects a null — no container is started and no insert is attempted. If all three files
  drop `NOT NULL` together, the check is silent by design, because that is a deliberate
  change and not drift.
- It reads the column line only. A nullability constraint expressed some other way — a
  table-level `CHECK`, an `ALTER TABLE ... SET NOT NULL` after the fact — is invisible to
  it. Nothing in these schema files does that today.
- Spec-mode objection **O3** — that a provisioned mysql or postgres database never gains
  these columns at all, because the schema files are applied only to the embedded H2 — is
  a different objection with the same number, and it remains deferred. Nothing here
  touches it. This fix makes the files agree with each other; it says nothing about what
  is running in a database somewhere.
...
carry the same hardcoded flash-message pattern in **seven** more places. This change
edits none of those files, so they stay outside the boundary. After this change one
of the application's **eight** flash messages is keyed and seven are not. That is a
real inconsistency, smaller than the one FR-20 closes, and it is recorded here rather
than left to be discovered.

**Corrected 2026-09-20, from code-mode objection O6.** This paragraph first said
"four more places" and "one of five is keyed and four are not". The count was wrong
and wrong in the flattering direction: it reached the four `message` attributes
(`OwnerController:85` and `:160`, `PetController:135` and `:177`) and missed three
`error` attributes rendered through the same page — `OwnerController:80`, `:148` and
`:154`. The full inventory is `grep -n addFlashAttribute src/main/java/.../owner/*.java`:
eight call sites, one keyed.

The miscount matters more than the three strings do. This file exists to be the
account a future reader trusts instead of re-deriving, and an undercount of the
residue is the one error it cannot afford — it makes the mess look smaller than it is
to exactly the person deciding whether to clean it up. It was found by an adversarial
read of the finished code, not by the review that passed it, and not by the author who
wrote the sentence while looking at the files.

## FR-18 orphaned the `date` key, and it is left in place

From code-mode objection O7. FR-18's third line moved the visit form's date label
from `#{date}` to `#{visitDate}`, and no template anywhere references `#{date}` any
more. The key is still there, translated, in `messages.properties` and all ten locale
bundles — `date=Date`, `date=Datum`, `date=Дата` and the rest.

That is the condition this same change refused to create one revision earlier. FR-19
dropped the `visit` key rather than write it "into eleven properties files with no
consumer", and then FR-18 orphaned a different key in the same sitting without
anyone noticing.

**Left in place deliberately.** The key costs nothing, a Date label is the most
likely thing a future form will want, and eleven deletions in bundles this change has
already reworked three times risks more than the tidiness is worth. It is recorded
here because nothing else can record it: `I18nPropertiesSyncTest` compares each
bundle's keys against the base bundle and reports what is *missing*. A key that no
template uses is invisible to it, permanently. So `date` is now indistinguishable, to
every tool in this project, from a key held in reserve — and this paragraph is the
